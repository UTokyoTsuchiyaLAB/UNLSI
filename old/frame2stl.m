function frame2stl(frame,stlname,HR,isWake,isStr)
    isMirror = frame.isSymm;
    verts = frame.verts.*HR;
    tri = frame.tri;
    if not(isWake == 1)
        tri(and(frame.surfID>200,frame.surfID<=300),:)=[];
        frame.surfID(and(frame.surfID>200,frame.surfID<=300),:)=[];
    end
    if not(isStr == 1)
        tri(frame.surfID>300,:)=[];
        frame.surfID(frame.surfID>300,:)=[];
    end
