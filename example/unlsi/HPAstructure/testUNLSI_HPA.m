...
[wing,weight] = wing.setFemMaterials([1,2,3,4,5,6,7,8],[0.0005,0.001,0.001,0.01,0.01,0.01,0.01,0.01],[5*1e9,9*1e9,9*1e9,5*1e9,5*1e9,5*1e9,5*1e9,5*1e9],[10,1600,1600,100,100,100,100,100],[100,100,100,100,100,100,100,100]);%物性値をセット 肉厚,ヤング率,密度,減衰パラメータ
disp("weight");
disp(weight);
wing = wing.makeFemEquation(); %FEM行列の作成
wing = wing.femModalAnalysis(15);
%%%%%%%%ここまでは一度計算すればスキップできる
%}
%
...
for iter = 1
    ...
    delta = wing.solveModalFem(wing.getCp(0).*Vinf.^2.*1.225.*0.5,1); %femを解く
