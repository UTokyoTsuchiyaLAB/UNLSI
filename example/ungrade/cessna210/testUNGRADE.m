...
lb = [-0.05,0.1,0,-0.05,0.1,4.0,-5,2,0]; %設計変数の下限値
ub = [ 0.05,0.14,6,0.05,0.14,8,5,8,6]; %設計変数の上限値
cmin = [45]'; %制約条件の下限値
cmax = [50]'; %制約条件の上限値
geomErr = 0.15;
...
for i = 1:60
    [nextVar,ungradetest] = ungradetest.calcNextVariables(@objFun,cmin,cmax,"TrustRegion",0.025,"betaLM",0.1);%次の設計変数を計算する
